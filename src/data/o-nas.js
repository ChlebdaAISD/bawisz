export const ONAS_FAQ = [
  {
    q: 'Czy w Bawisz są animatorzy, którzy zajmą się dzieckiem?',
    a: 'Nie. W codziennym wstępie do bawialni nie ma animatorów. Bawisz się ze swoim dzieckiem przy naturalnych zabawkach z drewna, my zajmujemy się salą, kawiarnią i wszystkim, co dookoła. Animatorzy i prowadzący pojawiają się tylko okazjonalnie lub  na zorganizowanych urodzinach (pakiet MINI 45 zł/os. lub STANDARD 74 zł/os.) i na warsztatach.',
  },
  {
    q: 'Co właściwie znaczy „bawialnia Montessori" w praktyce?',
    a: 'W praktyce to drewniana sala, w której wszystko jest na wysokości dziecka — półki, koszyki z zabawkami, kąciki tematyczne. Zabawki są z drewna, sklejki i tkanin, bez zbędnego plastiku. Dziecko samo wybiera, co robi, samo odkłada na miejsce. To filozofia samodzielności i naturalnych materiałów, nie marka mebli ani sztywny program zajęć.',
  },
  {
    q: 'W jakim wieku dzieci mogą tu przyjść?',
    a: 'Sala jest dla dzieci od 0 do 10 lat. Wnętrze dzieli się na strefy dopasowane do wieku — najmłodsze dzieci  bawią się w spokojniejszej części z miękkim podłożem, starsze szaleją na zjeżdżalniach i strefie z konstrukcjami. Opiekun zawsze gratis, drugie dziecko w rodzinie 25% taniej.',
  },
  {
    q: 'Czy do kawiarni potrzebny jest bilet do bawialni?',
    a: 'Nie. Kawiarnia działa osobno. Wpadasz na kawę bez dziecka albo z dzieckiem, które tego dnia nie chce się bawić — wchodzisz, zamawiasz przy ladzie, siadasz przy stoliku. Za wejście do bawialni zapłacisz tylko wtedy, gdy dziecko korzystało z sali zabaw(25 zł za 1 h, 29zł za 1,5h   33 zł za 2 h, 45 zł cały dzień, 215 zł karnet miesięczny no limit).',
  },
  {
    q: 'Gdzie jesteście i czy jest parking?',
    a: 'ul. Krzywa 19B, 34-400 Nowy Targ — centrum, dwie minuty od Rynku. Parking jest w pobliżu lokalu, bez problemu zaparkujesz nawet w sobotę po południu. Otwarte codziennie od 10:00 (w tygodniu do 19:00, w weekendy do 20:00).',
  },
  {
    q: 'Czy organizujecie urodziny, warsztaty i wejścia grupowe?',
    a: 'Tak — w trzech formatach. Urodziny w pakiecie MINI (45 zł/os., 2 h) lub STANDARD (74 zł/os., 2,5 h, sala tylko dla was, min. 10 dzieci, prezent dla solenizanta). Warsztaty — plastyczne, gimnastyka, zajęcia tematyczne w duchu Montessori. Wejścia grupowe dla przedszkoli i szkół od 15 zł za dziecko (min. 10 osób, kawa lub herbata gratis dla 1 opiekuna na 5 dzieci).',
  },
  {
    q: 'Jak najszybciej zarezerwować termin albo zapytać o szczegóły?',
    a: 'Telefon lub SMS na +48 693 766 049 — odpisujemy w godzinach otwarcia. Można też napisać na Instagramie (@bawisz_bawialnia) albo na Facebooku (BAWISZ Bawialnia Montessori). Wstęp do bawialni jest bez rezerwacji. Urodziny, warsztaty i grupy zorganizowane wymagają wcześniejszego ustalenia daty.',
  },
]

export const ONAS_META = {
  title: 'Bawialnia Montessori Nowy Targ | drewniana sala — Bawisz',
  description: 'Bawialnia Montessori w Nowym Targu — naturalna sala, drewniane zabawki, kawiarnia obok. Dzieci 0-10 lat. Wstęp od 25 zł. ul. Krzywa 19B. Ocena 4.9/5.',
  canonical: 'https://bawialniabawisz.pl/o-nas/',
  ogImage: 'https://bawialniabawisz.pl/assets/og/og-o-nas.webp',
}
